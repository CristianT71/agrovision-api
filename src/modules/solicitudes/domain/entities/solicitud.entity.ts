export type EstadoSolicitud = "Pendiente" | "Enviada" | "Asignada" | "Resuelta" | "Descartada";
...
export class Solicitud {
    ...
    public resolver(respuesta: string, tipoResultado: string): void {
        if (this.estado === "Resuelta") {
            throw new Error("La solicitud ya se encuentra resuelta y es de solo lectura.");
        }
    ...
    public asignarAgronomo(agronomoId: string): void {
        if (this.estado === "Resuelta") {
            throw new Error("No se puede reasignar una solicitud que ya fue resuelta.");
        }
